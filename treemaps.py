...
# For tree map plotting
#
import plotly
# OR import plotly.plotly as py
import plotly.graph_objs as graph_objs
# OR import plotly.graph_objs as go
...
def generate_treemaps(misp_data, num_days, bin_size, bin_name):
    """
    Generate treemaps for various criteria
    ...
    for html in [True, False]:
        ...
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-count-" + bin_name,
             "use_plotly": html
             })
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-levels-" + bin_name,
             "use_plotly": html
             })
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-ipsrc-" + bin_name,
             "use_plotly": html
             })
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-ipdst-" + bin_name,
             "use_plotly": html
             })
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-domains-" + bin_name,
             "use_plotly": html
             })
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-files-" + bin_name,
             "use_plotly": html
             })
        sets.append(
            {"num_days": num_days,
             ...
             "filename": "treemaps/treemap-analysis-" + bin_name,
             "use_plotly": html
             })
...
def generate_by_threat_actor(misp_data, num_days, bin_size, scoring_function, scoring_name, filename, use_plotly):
    """
    Generate a tree map for each threat actor, and save either as HTML or as a gnuplot input file and PNG

    misp_data - The events and attributes loaded from the MISP server
    num_days - The total number of days to generate the tree map for
    ...
    filename - The name of the file to write the treemap to
    ...
    if use_plotly:
        ...
        if not threat_actors:
            ...
            with open(filename + ".plt", "w") as outfile:
                ...
                # Don't draw a key because this is going to be a treemap
                outfile.write("unset key\n")

                # Produce a 2D treemap, not a 3D surface plot
                #
                ...
                # Set the treemap palette
                #
                outfile.write(
                    "set palette defined (0 \"black\", 0.33 \"red\", 0.66 \"yellow\", 1 \"white\")\n")

                # Now write out the data for the treemap
                #
                ...
                # End the data and plot the treemap
                #
                outfile.write("EOD\n")
